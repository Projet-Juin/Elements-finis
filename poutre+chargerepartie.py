...
F_repartie=[0 for i in range(2*N_element)] #va servir pour forces réparties : matrice colonne de 0 de taille 2*N_element : a remodifier pour que ca fonctionne

F_assemblee=[0 for i in range(2*N_element)] #va servir pour le systeme matrice colonne de 0 de taille 2*N_element : a remodifier pour que ca fonctionne
...
for i in range(N_element-1):
    ...
    if j=="encastrement": #dy=phi=0
        ...
        if chargepossible=='oui':
            print("charge :")
            charge = float(input())
            F_repartie[i*2]+=force_charge_uniformement_repartie(charge,listeabscisse[i+1]-listeabscisse[i])
            F_repartie[i*2+1]+=moment_charge_uniformement_repartie(charge,listeabscisse[i+1]-listeabscisse[i])

            z=type_appui[i+1]

            if z=="encastrement": #dy=phi=0
                F_repartie[i*2+2]+=force_charge_uniformement_repartie(charge,listeabscisse[i+1]-listeabscisse[i])
                F_repartie[i*2+3]+=moment_charge_uniformement_repartie(-charge,listeabscisse[i+1]-listeabscisse[i])

            if z=="rotule":    
                F_repartie[i*2+2]+=force_charge_uniformement_repartie(charge,listeabscisse[i+1]-listeabscisse[i])
                F_assemblee[i*2+3]+=moment_charge_uniformement_repartie(-charge,listeabscisse[i+1]-listeabscisse[i])

            if z=="rien":   
                 F_assemblee[i*2+2]+=force_charge_uniformement_repartie(charge,listeabscisse[i+1]-listeabscisse[i])
                 F_assemblee[i*2+3]+=moment_charge_uniformement_repartie(-charge,listeabscisse[i+1]-listeabscisse[i]) 
    ...
    if j=="rotule": #dy=0 et phi =/=0
        ...
        if chargepossible=='oui':
            print("charge :")
            charge = float(input())
            F_repartie[i]+=force_charge_uniformement_repartie(charge,listeabscisse[i+1]-listeabscisse[i])
            F_assemblee[i*2+1]+=moment_charge_uniformement_repartie(charge,listeabscisse[i+1]-listeabscisse[i])
    ...
    if j=="rien":    
        ...
        if chargepossible=='oui':
            print("charge :")
            charge = float(input())
            F_assemblee[i*2]+=force_charge_uniformement_repartie(charge,listeabscisse[i+1]-listeabscisse[i])
            F_assemblee[i*2+1]+=moment_charge_uniformement_repartie(charge,listeabscisse[i+1]-listeabscisse[i])
            z=type_appui[i+1]
...
while 0 in F_assemblee:                     #permet de supprimer tous les zeros de la liste utilisée pour le systeme
     del F_assemblee[F_assemblee.index(0)]
